...
依赖真相由 `scripts/deps.map` 记录, 用 pbuilder D-hook 校验编译时实际 `-dev` 版本. Debian upstream 若改 Build-Depends 必须人工更新此表. 复核方式: `https://sources.debian.org/src/<pkg>/sid/debian/control/`.
...
5. **pbuilder 本地测试编译** (含 D-hook 依赖版本校验), 缓存 chroot 到 `actions/cache@v4` (key 含 run_id 强制每天刷新, restore-key 前缀匹配复用)
...
**方案**: pbuilder D-hook (装完 build-dep 后运行) 里 `dpkg-query` 查每个关键 `-dev` 实际版本, 与 PPA 上游最新版本比较 (`dpkg --compare-versions`), 不足即 fail step, 不上传.
...
**方案**: 若 API 兼容 (通常小版本号变化只是打包 revision, 未真断 API), 在 dch 前 sed 掉该约束. 只保留包名, 让 noble 里旧版本满足.

**表**: `scripts/relax-deps.map`, 每行 `<package> <build-dep> [<build-dep> ...]`.
...
**当前放宽**:
- `librime`: `libmarisa-dev`, `libopencc-dev`, `libutfcpp-dev`, `libyaml-cpp-dev`
...
scripts/build-and-upload.sh           # 单包: 拉源→dch→debuild→pbuilder(带D-hook)→dput
...
**pbuilder D-hook fail**: hook 输出 "installed=X expected>=Y". 说明前 stage 上传成功但 apt cache 未刷新到. 检查 stage 之间 `wait-for-publish.sh` 是否成功等到 `Published` (含二进制).
